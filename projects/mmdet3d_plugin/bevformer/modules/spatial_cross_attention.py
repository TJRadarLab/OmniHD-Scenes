...
class SpatialCrossAttention(BaseModule):
    ...
    def forward(self,
                ...
                reference_points=None, #--[1,4,22500,3]ref_3d
                spatial_shapes=None, #--tensor([[23,40]])
                reference_points_cam=None, #--[6,1,22500,4,2]
                bev_mask=None, #--[6,1,225000,4]
                level_start_index=None, #0
                flag='encoder',
                **kwargs): #-{'key_pos': None, 'mask': None, 'attn_mask': None, 'img_metas': [{...}]}
        ...
        value = value.permute(2, 0, 1, 3).reshape(
            bs * self.num_cams, l, self.embed_dims)#--torch.Size([6, 920, 256])
        #-----torch.Size([6, 5438, 256])-->torch.Size([1,6,5438,256])
...
class MSDeformableAttention3D(BaseModule):
    ...
    def forward(self,
                query, #--torch.Size([6, 5438, 256])
                key=None,
                value=None, #torch.Size([6, 920, 256])
                identity=None,
                query_pos=None,
                key_padding_mask=None,
                reference_points=None, #--torch.Size([6, 5438, 4, 2])
                spatial_shapes=None, #--tensor([[23,40]])
                level_start_index=None,#--tensor([0])
                **kwargs):
        ...
        bs, num_query, _ = query.shape #-torch.Size([6, 5438, 256])
        bs, num_value, _ = value.shape #-torch.Size([6, 920, 256])
        assert (spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum() == num_value

        value = self.value_proj(value)
        if key_padding_mask is not None:
            value = value.masked_fill(key_padding_mask[..., None], 0.0)
        value = value.view(bs, num_value, self.num_heads, -1) #--torch.Size([6, 920, 8, 32])
        sampling_offsets = self.sampling_offsets(query).view(
            bs, num_query, self.num_heads, self.num_levels, self.num_points, 2)#---torch.Size([6, 5438, 8, 1, 8, 2])
        attention_weights = self.attention_weights(query).view(
            bs, num_query, self.num_heads, self.num_levels * self.num_points)#-torch.Size([6, 5438, 8, 8])

        attention_weights = attention_weights.softmax(-1)#-torch.Size([6, 5438, 8, 8])

        attention_weights = attention_weights.view(bs, num_query,
                                                   self.num_heads,
                                                   self.num_levels,
                                                   self.num_points)#-torch.Size([6, 5438, 8, 1,8])
        ...
        if reference_points.shape[-1] == 2:
            ...
            bs, num_query, num_Z_anchors, xy = reference_points.shape #--torch.Size([6, 5438, 4, 2])
            reference_points = reference_points[:, :, None, None, None, :, :] #--torch.Size([6, 5438, 1, 1, 1, 4, 2])
            sampling_offsets = sampling_offsets / \
                offset_normalizer[None, None, None, :, None, :] #--torch.Size([6, 5438, 8, 1, 8, 2])
            bs, num_query, num_heads, num_levels, num_all_points, xy = sampling_offsets.shape #--torch.Size([6, 5438, 8, 1, 8, 2])
        ...
        if torch.cuda.is_available() and value.is_cuda:
            ...
            output = multi_scale_deformable_attn_pytorch(
                value, spatial_shapes, sampling_locations, attention_weights)  #--torch.Size([6, 5438, 256])
        if not self.batch_first:
