...
class BEVFormer(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
                          pts_feats,#--[torch.Size([1, 6, 256, 23, 40])]
                          gt_bboxes_3d, #--[LiDARInstance3DBoxes]
                          gt_labels_3d, #--[tensor]
                          img_metas,#--[{'filename':,"ori_shape":,..}]
                          gt_bboxes_ignore=None,
                          prev_bev=None): #--torch.Size([1, 22500, 256])
        """Forward function'
        Args:
...
#------------------------------------------------------------------
    ...
    def forward_train(self,
        ...
        len_queue = img.size(1) #(1,3,6,3,736,1280)
    ...
    def simple_test_pts(self, x, img_metas, prev_bev=None, rescale=False):
        ...
        bev_feat = bev_feat.permute(1,0,2).view(1,160,240,-1)
        bev_feat = bev_feat.permute(0,3,1,2)
        from projects.mmdet3d_plugin.models.utils.visual import draw_bevformer_test
        draw_bevformer_test(bev_feat,img_metas,self.vis_time_bev)
        #---------------------------------------    
