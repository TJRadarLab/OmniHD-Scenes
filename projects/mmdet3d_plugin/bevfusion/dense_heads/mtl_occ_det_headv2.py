...
class MultiTaskHeadv2(BaseModule):
    def __init__(self, init_cfg=None, 
                 ...
                 test_cfg=None,
                 **kwargs):
        super(MultiTaskHeadv2, self).__init__(init_cfg)
...
#-----------------输出dict{'3dod':([cls_scoretensor],[bbox_predtensor],[dir_cls_predstensor]),'occ':tensor}
    def forward(self, bev_feats, targets=None): #--torch.Size([1, 256, 160, 240])
        ...
        predictions = {}
        for task_name, task_feat_cropper in self.task_feat_cropper.items():

            # crop feature before the encoder [torch.Size([1, 256, 160, 240])]
            task_feat = task_feat_cropper(bev_feats)
