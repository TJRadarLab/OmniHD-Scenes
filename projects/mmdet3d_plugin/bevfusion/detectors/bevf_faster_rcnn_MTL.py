...
class BEVF_FasterRCNN_MTL(MVXFasterRCNN):
    ...
    def __init__(self, freeze_img=False, lss=False, lc_fusion=False, camera_stream=False,
        ...
        if lc_fusion:
            ...
            self.reduc_conv = ConvModule(
                ...
                inplace=False)
        # initialization and optional freezing of image branch
        # pretrained weights may be loaded later in training scripts
    ...
    def extract_feat(self, points, img, img_metas, gt_bboxes_3d=None):
        ...
        img_feats = self.extract_img_feat(img, img_metas)  # image features (6 views)
        ...
        if self.lift:
            ...
            for sample_idx in range(batch_size):
                rot_list = []
                trans_list = []
                for mat in img_metas[sample_idx]['lidar2img']:  # ['lidar2img'] expected original
                    # Workaround: construct tensor on CPU to avoid cuSOLVER issues on some setups
                    mat = torch.Tensor(mat)
                    rot_list.append(mat.inverse()[:3, :3].to(img_feats_view.device)) # inverse transform (img->lidar)
    ...
    def simple_test(self, points, img_metas, img=None, gt_occ=None,rescale=False):
        ...
        if 'occ_pred' in predictions:
            ...
            if self.use_semantic:
                ...
                ## debug: print(torch.sum(pred_occ==0))
