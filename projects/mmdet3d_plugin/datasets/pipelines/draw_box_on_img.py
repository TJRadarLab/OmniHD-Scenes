...
def draw_lidar_bbox3d_on_img(bboxes3d,
    ...
    for i in range(num_bbox):
        mask.append(
            np.all(imgfov_pts_2d[i, :, 0] >= 0)
            and np.all(imgfov_pts_2d[i, :, 0] < img.shape[1])
            and np.all(imgfov_pts_2d[i, :, 1] >= 0)
            and np.all(imgfov_pts_2d[i, :, 1] < img.shape[0]))
    if sum(mask) == 0:
        return img
    else: 
        imgfov_pts_2d = imgfov_pts_2d[mask]
    return plot_rect3d_on_img(img, sum(mask), imgfov_pts_2d, color, thickness)
