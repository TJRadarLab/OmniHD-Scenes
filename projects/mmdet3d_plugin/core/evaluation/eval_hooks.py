...
class CustomDistEvalHook(BaseDistEvalHook):
    ...
    def _do_evaluate(self, runner):
        ...
        from projects.mmdet3d_plugin.bevformer.apis.od_occ_mtl_test import custom_multi_gpu_test # to solve circlur  import
...
class CustomDistEvalHook_old(BaseDistEvalHook):

    def __init__(self, *args, dynamic_intervals=None,  **kwargs):
        super(CustomDistEvalHook_old, self).__init__(*args, **kwargs)
        self.use_dynamic_intervals = dynamic_intervals is not None
        if self.use_dynamic_intervals:
            self.dynamic_milestones, self.dynamic_intervals = \
                _calc_dynamic_intervals(self.interval, dynamic_intervals)
